...
export default function AvaliarCorridaMotoboy() {
  ...
  const enviar = async () => {
    if (!corrida?.passageiro?.id) return
    setLoading(true)
    try {
      await api.post(`/corridas/${corridaId}/avaliar`, {
        avaliadoId: corrida.passageiro.id,
        nota,
        comentario,
      })
  ...
  const pular = () => navigate('/motoboy/dashboard')
      ...
      <div className="card anim-up" style={{ width: '100%', maxWidth: 400, textAlign: 'center', padding: 32 }}>
        ...
        {passageiro && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, margin: '16px 0' }}>
            <img
              src={passageiro.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(passageiro.name || 'P')}&background=1a253e&color=fff`}
              alt={passageiro.name}
              style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover' }}
            />
            <div style={{ textAlign: 'left' }}>
              <div style={{ fontWeight: 700, color: 'var(--text)' }}>{passageiro.name}</div>
              <div style={{ fontSize: 13, color: 'var(--text3)' }}>Passageiro</div>
            </div>
          </div>
        )}

        <p style={{ color: 'var(--text3)', marginBottom: 20, fontSize: 14 }}>100% para você. Como foi o passageiro?</p>
