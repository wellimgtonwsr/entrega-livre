...
const s = {
  ...
  valor: { fontWeight: 700, fontSize: 18, color: '#1a1a2e' },
...
export default function HistoricoCliente() {
  ...
  return (
    <div style={s.wrap}>
      <div style={s.header}>
        <div style={s.title}>Histórico 📋</div>
      </div>
      <div style={s.body}>
        {loading && <div style={s.empty}>Carregando...</div>}
        {!loading && pedidos.length === 0 && <div style={s.empty}>Nenhum pedido ainda</div>}
        {pedidos.map(p => {
            ...
            <div key={p.id} style={s.card}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {p.status === 'IN_PROGRESS' && (
                  <button onClick={() => navigate(`/cliente/acompanhar/${p.id}`)} style={{ background: '#1a1a2e', color: '#fff', border: 'none', borderRadius: 10, padding: '8px 14px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                    Acompanhar
                  </button>
                )}
              </div>
