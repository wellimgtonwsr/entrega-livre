  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100dvh', background: 'var(--bg)' }}>
      <div className="page-header">
        <div className="page-header-title">Histórico 📋</div>
      </div>
          ...
          return (
            <div key={p.id} className="card" style={{ marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ fontWeight: 700, fontSize: 18, color: 'var(--text)' }}>R$ {(p.valorFinal ?? p.valorProposto).toFixed(2)}</div>
                {p.status === 'IN_PROGRESS' && (
                  <button onClick={() => navigate(`/cliente/acompanhar/${p.id}`)} className="btn btn-sm" style={{ background: 'var(--dark)', color: '#fff' }}>
                    Acompanhar
                  </button>
                )}
              </div>
